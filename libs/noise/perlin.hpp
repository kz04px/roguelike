#ifndef NOISE_PERLIN_HPP
#define NOISE_PERLIN_HPP

#include <cmath>

namespace {

constexpr const int p[512] = {
...
constexpr double fade(const double t) {
    return t * t * t * (t * (t * 6 - 15) + 10);
}

constexpr double lerp(const double t, const double a, const double b) {
    return a + t * (b - a);
}

constexpr double grad(const int hash,
                      const double x,
                      const double y,
                      const double z) {
    const int h = hash & 15;
...
constexpr double get(double x, double y, double z) {
